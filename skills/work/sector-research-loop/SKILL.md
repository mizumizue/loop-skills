...
テーマ語の連想や業種一般論で止めず、**sector brief** を一次に近い根拠まで掘る。仕様・PBI・実装には進まない。後工程（仕様深度・デザイン等）の入力になる。
...
| 起動 | 人間との接点 |
|------|----------------|
| 上位オーケストレータの1工程（例: `demo-app-loop-workflow`） | **なし**。テーマ・`STATE_DIR`・打ち切りは呼び出し側の指示書に従う。判断はオーケストレータに任せる |
| ユーザーが単体起動 | 起動文の指示だけ使う。以降は確認しない。指示が薄い項目は AI が決め、聞かない |

既存の `research`（公式ドキュメント／API 一次情報）とは別。本スキルは **業態・現場オペ・通例** 向け。
...
| V4 | 次がそれぞれ記述されている: ロール／権限差、提供物の可変（追加・削除・構成の通例）、主取引で決める項目、オプション×所要（時間・負荷・価格など）、現場オペ |
...
2. Generator: 業態に即した一次に近いソース（業界資料、公開オペ手順、受付／発注／予約などの実務面、規制・通例の記述など）を当たり、事実／仮説を分けて `sector-brief.md` を書く／直す。最低欄は業種・業態・顧客属性・現場オペ・通例・ロール権限・提供物の可変・主取引の決定項目・オプション×所要・採用／見送り  
